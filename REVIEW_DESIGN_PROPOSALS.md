...
- **Step C** ✅ done (`772bc63`, 2026-07-29) — `score_element` now builds a `Vec<Signal>` and aggregates via a new `additive_sum` (not `weighted_sum` — see the correction below), keeping the exact same coefficients. Achieved better than the "tolerate negligible drift" bar originally set: behavior is preserved exactly (all 124 intent tests + 17 scoring tests pass unchanged; live-verified identical top-ranked symbol/relevance% before and after).
- **Step D** (open, first real behavior change; ship with item 3's `BlendPolicy` review): thread `ScoringContext` through the intents call site too so intents gain blend and both surfaces gain a shared exclusion set. `RelevanceScorer::score_element` doesn't yet implement the `Scorer` trait itself (it takes `params` as a per-call argument, not scorer-instance state) — Step D will need to decide how `RelevanceScorerParams` and a `Scorer` impl coexist, e.g. a small wrapper struct that owns the params.

**Second refinement to the original design (found during Step C implementation)**: `score_element`'s formula is a **sum of independent, stacking bonuses and penalties** (concept-type weight, doc/visibility boosts, complexity/name-clarity adjustments — several of them negative), not a **weighted average** of competing signals like lenses' single glob-priority value. Feeding it through `weighted_sum` (which divides by total weight) would have changed the actual score, since 5 signals averaged is a very different number than 5 terms summed. Added `additive_sum` (plain `Σ value*weight`, no division) as a second aggregation primitive in `src/core/scoring` — `weighted_sum` for domains where signals compete/average, `additive_sum` for domains where signals are independent and stack.
