...
**2.2 One parser on the main path** (I6, I3) — direction confirmed, DECISIONS Q1/Q4 (High confidence)
...
**2.4 One selection engine** (C5, I2, I8) — **AMENDED by DECISIONS Q2 (High confidence); needs a design pass before execution**
...
- **3.3 Invest in `report_utility`**: it's the moat — a feedback loop from the consuming LLM back into selection priors. DECISIONS Q9 (High confidence, exhaustive trace): confirmed **write-only today** — `blend_priority` only runs when `LensManager` has a `ContextStore`, and production code never constructs one with a store; the read-back path is "~one line from working" (`with_store(ContextStore::load_from_file(...))`). **Decision: wire it.** But this needs its own small design pass, not straight execution: add decay/exploration logic so early low scores don't permanently bury a file (a bare linear blend would let one bad early score sink a file forever) — treat it as the same "needs a design proposal first" caution as 2.4, just smaller in scope.
