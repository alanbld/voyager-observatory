...
"#;
    ...
    assert!(
        metrics.stars.count >= 3,
        "Should find at least 3 Simula stars, found {}",
        metrics.stars.count
    );
    assert!(
        metrics.stars.types >= 2,
        "Should find at least 2 classes, found {}",
        metrics.stars.types
    );
...
"#;
    ...
    assert_eq!(
        metrics.stars.count, 5,
        "Should find exactly 5 Logo procedures, found {}",
        metrics.stars.count
    );
    assert_eq!(
        metrics.stars.functions, 5,
        "All Logo stars should be functions"
    );
...
"#;
    ...
    assert_eq!(
        metrics.stars.count, 6,
        "Should find exactly 6 Tcl procedures, found {}",
        metrics.stars.count
    );
...
"#,
    ...
    assert!(
        total_stars >= 6,
        "Should find at least 6 total stars, found {}",
        total_stars
    );
}
...
fn test_extension_detection() {
    // Simula
    assert!(
        STELLAR_LIBRARY.get_by_extension("sim").is_some(),
        "Should recognize .sim extension"
    );

    // Logo
    assert!(
        STELLAR_LIBRARY.get_by_extension("logo").is_some(),
        "Should recognize .logo extension"
    );
    assert!(
        STELLAR_LIBRARY.get_by_extension("lg").is_some(),
        "Should recognize .lg extension"
    );

    // Tcl
    assert!(
        STELLAR_LIBRARY.get_by_extension("tcl").is_some(),
        "Should recognize .tcl extension"
    );
    assert!(
        STELLAR_LIBRARY.get_by_extension("tk").is_some(),
        "Should recognize .tk extension"
    );

    // COBOL
    assert!(
        STELLAR_LIBRARY.get_by_extension("cob").is_some(),
        "Should recognize .cob extension"
    );
    assert!(
        STELLAR_LIBRARY.get_by_extension("cbl").is_some(),
        "Should recognize .cbl extension"
    );

    // Fortran
    assert!(
        STELLAR_LIBRARY.get_by_extension("f90").is_some(),
        "Should recognize .f90 extension"
    );
    assert!(
        STELLAR_LIBRARY.get_by_extension("f95").is_some(),
        "Should recognize .f95 extension"
    );
}
...
fn test_hemisphere_classification() {
    ...
    for lang in automation_langs {
        ...
        assert_eq!(
            sig.hemisphere,
            Hemisphere::Automation,
            "{} should be in Automation hemisphere",
            lang
        );
    }
    ...
    for lang in interface_langs {
        ...
        assert_eq!(
            sig.hemisphere,
            Hemisphere::Interface,
            "{} should be in Interface hemisphere",
            lang
        );
    }
...
fn test_stellar_library_tiers() {
    ...
    for lang in tier1 {
        assert!(
            library.get(lang).is_some(),
            "Tier 1 Giant '{}' should be supported",
            lang
        );
    }
    ...
    for lang in tier2 {
        assert!(
            library.get(lang).is_some(),
            "Tier 2 Infrastructure '{}' should be supported",
            lang
        );
    }
    ...
    let tier3 = [
        "cobol", "simula", "logo", "fortran", "pascal", "lisp", "prolog", "ada",
    ];
    for lang in tier3 {
        assert!(
            library.get(lang).is_some(),
            "Tier 3 Ancient Star '{}' should be supported",
            lang
        );
    }
    ...
    let tier4 = [
        "haskell", "elixir", "erlang", "clojure", "scala", "fsharp", "ocaml",
    ];
    for lang in tier4 {
        assert!(
            library.get(lang).is_some(),
            "Tier 4 Functional '{}' should be supported",
            lang
        );
    }
    ...
    for lang in tier5 {
        assert!(
            library.get(lang).is_some(),
            "Tier 5 Emerging '{}' should be supported",
            lang
        );
    }
    ...
    for lang in tier6 {
        assert!(
            library.get(lang).is_some(),
            "Tier 6 Scientific '{}' should be supported",
            lang
        );
    }
}
...
fn test_stellar_library_language_count() {
    ...
    assert!(
        count >= 60,
        "Should have at least 60 languages, found {}",
        count
    );
...
fn test_all_star_patterns_valid() {
    ...
    for lang in library.languages() {
        ...
        assert!(
            result.is_ok(),
            "Invalid star pattern for {}: {} - Error: {:?}",
            lang,
            sig.star_pattern,
            result.err()
        );
...
fn test_all_comment_patterns_valid() {
    ...
    for lang in library.languages() {
        ...
        assert!(
            single_result.is_ok(),
            "Invalid single-line comment pattern for {}: {}",
            lang,
            sig.comment_single
        );
        ...
        assert!(
            multi_start_result.is_ok(),
            "Invalid multi-line comment start for {}: {}",
            lang,
            sig.comment_multi_start
        );

        // Multi-line comment patterns (end)
        let multi_end_result = regex::Regex::new(sig.comment_multi_end);
        assert!(
            multi_end_result.is_ok(),
            "Invalid multi-line comment end for {}: {}",
            lang,
            sig.comment_multi_end
        );
    }
}
