...
pub use composition::{
    ConfiguredPrimitive, ExplorationIntent, ExplorationStep, IntentComposition, IntentResult,
};

pub use decisions::{ReadingDecision, StopReadingEngine};

pub use explorer::{ExplorationResult, ExplorerConfig, IntentExplorer};

#[cfg(test)]
mod tests {
    use super::*;
    use crate::core::fractal::{
        ContextLayer, LayerContent, Range, SymbolKind, SymbolVectorizer, Visibility,
    };
    ...
    fn test_noise_filter_removes_tests() {
        ...
        for (idx, _concept_type) in &filtered {
            ...
            assert!(
                ...
                "Should not contain test_ layers"
            );
    ...
    fn test_intent_composition_end_to_end() {
        ...
        assert!(result.estimated_minutes > 0, "Should estimate reading time");
    }
    ...
    fn test_different_intents_produce_different_results() {
        ...
        let business_top = business_result
            .exploration_path
            .first()
            .map(|s| s.symbol.clone());
        let debugging_top = debugging_result
            .exploration_path
            .first()
            .map(|s| s.symbol.clone());
